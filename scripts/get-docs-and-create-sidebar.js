...
function getTSXfilesMap(folder) {
  let files = fs.readdirSync(folder);
  let tsxMdxExists = checkForMDXandTSXFilesInPath(folder);
  let flag = false;
      ...
      if (tsxMdxExists) {
        ...
        if (
          ...
          if (file.endsWith(".web.tsx")) {
            if (
              tsxFilesMap.hasOwnProperty(lowerCaseFileName.replace(".web", ""))
            ) {
        ...
        if (stats.isDirectory() && flag == false) {
  ...
  if (canvasStoryDataString) {
    ...
    for (let i = 0; i < canvasStoryDataString.length; i++) {
      ...
      if (idDataStringValue) {
        ...
        let mappedTsxFilePath =
          tsxFilesMap[getCodeFileName] ||
          tsxFilesMap[getCodeFileName.replace("story", "")];

        if (mappedTsxFilePath) {
          let codeDataString = fs.readFileSync(mappedTsxFilePath, "utf-8");
...
function checkForMDXandTSXFilesInPath(folderPath) {
  let hasMdx = false;
  let hasTsx = false;

  let files = fs.readdirSync(folderPath);
  for (let i = 0; i < files.length; i++) {
    if (files[i].endsWith(".mdx")) {
      hasMdx = true;
    } else if (files[i].endsWith(".tsx")) {
      hasTsx = true;
    }
  }
  if (hasMdx && hasTsx) {
