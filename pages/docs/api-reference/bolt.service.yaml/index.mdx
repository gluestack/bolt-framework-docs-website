...
This is an array of strings. Each string contains a service runner name. You can use `local`, `docker`, `vmlocal` or `vmdocker` as your service runner name.

Supported Service Runners stands for the service runners that your service supports. For example, if you use `local` and `docker` as your supported service runners, your service will be able to run on your host machine's local and Docker.
